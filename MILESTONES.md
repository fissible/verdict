...
| [#297](https://github.com/fissible/verdict/issues/297) `RequireReview` substrate (the keystone) | L–XL | none | open — `scope: ready`; substrate build underway, dependency-ordered: ADR 0035 (#428), the loud reserve (#429), the value layer (#434), the review-request store (#439), the fail-closed decision authorizer (#440), and the status read surface (#441) have landed, and the execution gate is underway — its issuance half (`ReviewManager::issue`) has landed; the database store, the `runBound()`/`run()` wiring that replaces #429's throw, and the admission gate remain |
