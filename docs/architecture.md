...
The tool definition describes the model-facing contract. The capability defines application authority: the Laravel authorization ability, trusted target resolution, optional safeguards, and the code performing the side effect.
...
The seven steps above are the orientation-level summary. Gate ordering is fully decided across [ADR 0001](adr/0001-semantic-execution-rate-limits.md), [ADR 0002](adr/0002-strict-at-most-once-admission.md), [ADR 0003](adr/0003-execution-target-freshness.md), and [ADR 0004](adr/0004-independent-security-state-transactions.md); this table is a navigation aid over those decisions, not a restatement that could drift from them. For the exact order, read [ADR 0003 §"Required execution order"](adr/0003-execution-target-freshness.md#required-execution-order); this table exists so a reader need not assemble it from four documents.
...
| 9.5 | Commit the write-ahead intent record, if the intent lever is effective for this capability — a failed write denies with nothing consumed | [ADR 0007](adr/0007-evidence-layering.md) Update ([#160](https://github.com/fissible/verdict/issues/160)) |
...
for that finalization fails, it does not reach the caller: an `EvidenceWriteFailed` event is
...
A numbered step list is also, by construction, a behavioral argument — correct only while nobody reorders the statements. [ADR 0013](adr/0013-authorization-binding-layers.md) states the property this ordering produces as a state-based invariant instead (**Invariant B1**: no execution-stage decision, approval transition, rate-limit consumption, or execution-claim admission may be derived from a target snapshot older than the refresh performed for that envelope) — something a reviewer or test can check directly, independent of step order. Read the table for *what happens when*; read ADR 0013 for the property a refactor of this order must still preserve.

## Target strategies

`ExecutionTargetPolicy::refresh()` records canonical identity and re-fetches the target at execution time. Prefer it for mutable records. `acceptStaleSnapshot()` deliberately uses the original resolved object; use it only when stale-snapshot behavior is acceptable.
...
Outside the `local` and `testing` environments it also warns for each non-durable adapter it finds configured — the in-memory evidence recorder and the in-memory approval, rate-limit, execution-claim, and capability-configuration stores — naming the config key that selects each and the consequence specific to it. A process-local rate limit binds per process, so N workers admit up to N times the configured limit; a process-local claim store degrades at-most-once to at-most-once-per-process. These are **warnings and do not change the exit code**, because Verdict does not decide an application's deployment topology and an ephemeral preview environment may legitimately run one. Pass `--strict` to make CI block on them along with every other advisory finding. The environment test is the framework's own, so any environment not named `local` or `testing` is covered without configuring a list of production names.
...
`GuardedTool` and `Verdict::guard(...)` are a bounded migration bridge, to move an application’s existing pre-Verdict Laravel AI tools onto Verdict’s authorization boundary without rewriting them. They authorize a resolved target and then delegate to an independent handler, so Verdict cannot establish that the handler acts on the same target.

Do not use them for new security-sensitive capabilities. `GuardedTool` also does not support verified confirmation, because its independent handler cannot be bound to the authorized target. Use `BoundTool` for new capabilities and migrate security-sensitive actions to it. The rationale and boundary are in [ADR 0005](adr/0005-guardedtool-is-a-bounded-migration-bridge.md).
...
Verdict does not duplicate what Laravel AI already owns. It adds an authorization boundary in front and leaves transport, generation, and conversation mechanics where they are.
