...
5. **Streamed approval resumption is verified up to the resume, not through completion.** `StreamedApprovalResumptionTest` drives a confirmation-gated capability through Laravel AI's real `stream()` pipeline against a faked conversational agent using the shipped `DatabaseConversationStore`, and asserts three things: the run **pauses** for confirmation, the executor **does not run** before approval, and resuming with `Decision::approveAll()` **does not raise** — the `ApprovalNotResumableException` a non-conversational agent produces. What it deliberately does not assert is that the executor runs *after* the resume: under `Agent::fake()` it does not fire, and whether that is the product or the fake is unresolved, because a fake supplies provider output while a resume replays a pending tool call from conversation history rather than asking the model again. Asserting completion against the fake would pin whichever answer the fake happens to give. That half needs a live provider and is tracked in [#218](https://github.com/fissible/verdict/issues/218). One wiring precondition is load-bearing and easy to miss: `VerdictManager::requestConfirmation()` returns `null` for a capability that declares `requiresConfirmation()` but no execution-target policy, so such a capability never pauses at all — see [#230](https://github.com/fissible/verdict/issues/230).
