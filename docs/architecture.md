...
| Confirmation / approval resumption | ✅ | ⚠️ Not verified⁵ — [ADR 0006](adr/0006-streaming-approval-resumption-deferred.md), frame lifetime fixed in #22 | Not yet verified³ |
...
5. **Streamed approval resumption is not verified, and this cell previously said it was.** No test in the suite exercises it: `approveAll`, `hasPendingApprovals`, and `requireApproval` appear only in `ApprovalFlowTest`, which mocks the agent and never streams, and `StreamedExecutionGatesTest` does not touch approvals at all. Measured against two live providers — Ollama `qwen2.5-abliterate:7b` and Anthropic `claude-haiku-4-5`, both of which genuinely called the tool — the tool executed instead of pausing: `hasPendingApprovals` was `false` in both runs, so there was no pending call for `stream(Decision::approveAll())` to resume. Whether `AbstractVerdictTool::shouldRequestApproval()` was reached at all is not yet established — the evidence rows those runs produced are emitted by `runBound()` during execution and do not distinguish the two. Verdict then denied at the execution stage, which is fail-closed and correct — the protected action did not run — but the round trip the cell claimed does not currently happen under streaming. What #22 fixed is real and narrower: `VerdictApprovalMiddleware` keeps `ApprovalExecutionContext`'s frame alive through a streamed response's full iteration. Whether the remaining gap is upstream, in Verdict's adapter, or a wiring precondition is unresolved and tracked in [#227](https://github.com/fissible/verdict/issues/227). **The synchronous cell is affected too, and this footnote first said it was not.** A `prompt()` run against Ollama with the same capability produced the identical result — the tool executed, `hasPendingApprovals` was `false`, and no pause occurred. The failure is mode-independent, so the synchronous ✅ above rests on `ApprovalFlowTest`, which mocks the agent, and on deterministic scenario replay; no live provider has demonstrated a pause in any mode. Verdict's approval machinery itself is exercised and works when driven directly — what is unverified is the integration that turns a required confirmation into an agent-loop pause.
