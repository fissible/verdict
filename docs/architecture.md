...
3. **Queued approval resumption is not yet verified, and the reason previously given here was wrong.** This note used to say the blocker was that `InvokeAgent` "does not itself retain the initial job's pending tool-call response." A resume does not read that response: `TextGenerationLoop::pendingToolCalls()` reconstructs the pending call from **conversation history**, and the synchronous path passes `resumableApprovalFor($prompt)` into `generate()` exactly as the streamed path does — so `Agent::queue(Decisions)` → `InvokeAgent` → `prompt(Decisions)` has the mechanism available to it. The actual gap is coverage: `QueuedExecutionGatesTest` drives its agent with `Agent::fake()`, which can never resume tools (see note 5), so verifying this cell needs a `StepTextGateway` implementing `generateTextStep` plus a durable `ConversationStore` surviving the job boundary. Tracked in [#234](https://github.com/fissible/verdict/issues/234). Until that lands, treat queued approval resumption as unverified — not as known-broken.
