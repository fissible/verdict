...
- [ ] The selected execution mode is marked verified in the [compatibility matrix](architecture.md#execution-mode-compatibility), and the test boundary in its note is acceptable. In particular, **no execution mode currently has a live-verified approval pause** ([#227](https://github.com/fissible/verdict/issues/227)). Queued needs application-owned durable conversation state; streamed and synchronous were both measured against a live provider and executed the tool rather than pausing. Verdict still fails closed in every measured case — the confirmation-gated action was denied and did not run — so the risk is a capability that refuses rather than one that leaks. Do not build a high-consequence approval flow on the agent-loop pause until #227 resolves; drive confirmation from your own application flow (issue the challenge, obtain the receipt, then invoke) rather than relying on the model loop to pause.
