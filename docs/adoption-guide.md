...
- [ ] The selected execution mode is marked verified in the [compatibility matrix](architecture.md#execution-mode-compatibility), and the test boundary in its note is acceptable. In particular, **neither streamed nor queued approval resumption is verified**: queued needs application-owned durable conversation state, and streamed does not currently pause at all ([#227](https://github.com/fissible/verdict/issues/227) — measured against two live providers). Do not select either for a high-consequence approval flow. Synchronous approval resumption is exercised by the suite and is the mode to use for confirmation-gated capabilities today.
