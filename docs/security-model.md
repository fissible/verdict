...
`approved_by` (and `rejected_by`) is **attestation by the application**, not authentication by Verdict: the manager persists the identifier string the application supplies and verifies nothing about it. What makes it mean something is the required `ApprovalDecisionAuthorizer` (`verdict.approvals.authorizer`): `approve()`/`reject()` are fail-closed and refuse every decision — `ApprovalAuthorizerMissing` — until the application configures one, and return `unauthorized` without touching the receipt when it denies. The authorizer answers the one question a shipped package cannot: may *this* decision maker finalize *this* receipt? Where the receipt store declares `EnforcesDecisionAdmissibility` — as both shipped stores do — the authorizer is asked only about a receipt that is still decidable: matching, pending, and unexpired. A receipt that has already expired or been decided reports its own state (`expired`, `invalid_state`) and is never put to the authorizer, so an operator holding a stale receipt reads why it is stale rather than an authorization failure. A custom store that has not declared it, or a decorator that wraps one without re-declaring, gets the older and stricter order instead: every receipt the decision addresses is authorized first, whatever its state, so a store that does not refuse an inadmissible decision is never handed one. See [ADR 0036](adr/0036-receipt-state-precedes-authorization.md) and its #436 update. Refusing for want of an authorizer comes first regardless: `ApprovalAuthorizerMissing` is thrown whatever the receipt's state.
