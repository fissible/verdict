...
`approved_by` (and `rejected_by`) is **attestation by the application**, not authentication by Verdict: the manager persists the identifier string the application supplies and verifies nothing about it. What makes it mean something is the required `ApprovalDecisionAuthorizer` (`verdict.approvals.authorizer`): `approve()`/`reject()` are fail-closed and refuse every decision — `ApprovalAuthorizerMissing` — until the application configures one, and return `unauthorized` without touching the receipt when it denies. The authorizer answers the one question a shipped package cannot: may *this* decision maker finalize *this* receipt? Where the receipt store declares `EnforcesDecisionAdmissibility` — as both shipped stores do — the authorizer is asked only about a receipt that is still decidable: matching, pending, and unexpired. A receipt that has already expired or been decided reports its own state (`expired`, `invalid_state`) and is never put to the authorizer, so an operator holding a stale receipt reads why it is stale rather than an authorization failure. A custom store that has not declared it, or a decorator that wraps one without re-declaring, gets the older and stricter order instead: every found, call-matching receipt is authorized first, whatever its state, so a store that does not refuse an inadmissible decision is never handed one. A missing or call-mismatched receipt is delegated without authorization under either order — the store answers `not_found` or `mismatch` and finalizes nothing. See [ADR 0036](adr/0036-receipt-state-precedes-authorization.md) and its #436 update. Refusing for want of an authorizer comes first regardless: `ApprovalAuthorizerMissing` is thrown whatever the receipt's state.
...
Decision evidence records the fingerprint of the tool description Verdict was wired with (`tool_description_fingerprint`), the fingerprint of the description advertised to the model for that invocation (`invocation_tool_description_fingerprint`), and the comparison itself (`tool_description_matched`). A divergence is the signal that a tool's advertised description changed after binding — description poisoning, or an accidental mutation of a shared tool instance.

`tool_description_matched` is stored rather than left to be recomputed, and it is indexed, because an operator reviewing an incident should be able to *find* divergences without knowing to compare two columns. It is **null when the description was never advertised** — a tool invoked without a prompt build was not observed, and reporting that as a match would claim an observation nobody made. The advertised fingerprint is scoped to its invocation and shared by every call the model makes within it, including parallel calls to the same tool; it is released only when that invocation fully unwinds. Laravel AI maps every tool description into each provider request, so a re-advertised invocation is observed again with its own fingerprint and only an un-advertised one records none.
