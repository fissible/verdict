...
| `ApprovalReceiptStore` | Stable | `DatabaseApprovalReceiptStore`, `InMemoryApprovalReceiptStore` | Store approval receipts in a durable or external backend. A store must refuse `approve()`/`reject()` for any receipt that is not call-matching, `Pending`, and unexpired at the supplied instant — that is what makes its outcomes canonical. Whether `ApprovalManager` *relies* on it doing so is a separate, opt-in question: see `EnforcesDecisionAdmissibility` below (ADR 0036, #436). |
...
| `EnforcesDecisionAdmissibility` | Experimental | `DatabaseApprovalReceiptStore`, `InMemoryApprovalReceiptStore` | Opt-in beside `ApprovalReceiptStore` (#436): the store states that `approve()`/`reject()` atomically refuse any receipt that is not call-matching, `Pending`, and unexpired. `ApprovalManager` then skips the decision authorizer for an inadmissible receipt and lets the store report its own state (ADR 0036). A store that does not declare it gets the older, stricter order — every addressable receipt is authorized before anything is delegated — so a store that does not refuse an inadmissible decision is never handed one. Verdict cannot verify the claim; a decorator that does not re-declare falls back to the stricter path, which is the safe direction. |
