...
Every durable security-state mutation retries once after a randomized 10–50 ms delay when a transaction raises SQLSTATE `40001` (serialization failure) or `40P01` (deadlock), in addition to any unique-constraint-violation handling it already has. That deliberately excludes MySQL's lock-wait timeout (1205): retrying after its potentially long synchronous wait would double execution latency rather than resolving a short-lived conflict. The policy covers `DatabaseRateLimitStore::consume()`, every `DatabaseExecutionClaimStore` transition (`claim()`, `complete()`, `markIndeterminate()`, `resolve()`), and every `DatabaseApprovalReceiptStore` transition (`issue()`, `approve()`, `reject()`, `consume()`) — the retry is paired with ADR 0004's independent-transaction guard in one call, so a mutation cannot acquire one without the other. The delay is measured with genuine, synchronized process-level concurrency: a ready/release handshake between child processes, not a fixed wall-clock guess.
