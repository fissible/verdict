...
**Spec coverage:** every requirement in issue #22's "What's needed" section maps to a task — extending the frame's lifetime across `StreamableAgentResponse` consumption (Task 2), a regression test driving a `Decisions`-carrying prompt through a streamed response and confirming it no longer fails closed with `InvalidState` (Task 2), and the doc/ADR update issue #22 itself says is already in flight (Task 3).

**Post-merge correction (PR #78 review, 2026-08-10):** the item above originally claimed the issue's "verify this against `StreamedAgentResponse::withPendingApprovals()` / `pausedProviderContentBlocks()` so a nested pause mid-resumption is still handled correctly" requirement was "a non-issue by construction," reasoned but never actually tested. Review correctly rejected that as insufficient: the generator wrapper being a pure `yield from` passthrough is a claim about the fix's own code, not proof that Laravel AI's `StreamedAgentResponse` construction — which derives `pendingApprovals` and `pausedProviderContentBlocks()` from `ToolApprovalRequest` events specifically — actually receives those events intact. None of Task 2's tests exercised a `ToolApprovalRequest` event; they all used `StreamEnd` only. Added a test resuming an already-approved call, then yielding a real `ToolApprovalRequest` (with a `PendingApproval` and provider replay blocks) before completing, asserting via a `then()` callback that the resulting `StreamedAgentResponse` carries both correctly while the approved call still executed. This is the standing lesson: "handled correctly by construction" is not itself a substitute for a test that drives the actual code path, even when the reasoning is sound.
